//! Adds 2 single-precision vectors.
//!
//!     z := x + y
//!
...
comptime {
    if (@import("builtin").target.cpu.arch.isSpirV()) {
        @export(&addveci32, .{ .name = "addveci32" });
    }
}
...
const VkArray = @SpirvType(.{ .runtime_array = i32 });
...
fn addveci32() callconv(.{ .spirv_kernel = .{ .x = WgSize.x, .y = WgSize.y, .z = WgSize.z } }) void {
